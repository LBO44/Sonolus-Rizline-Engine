...
if (process.env.MODE === "dev") {
  ...
  const shouldUpdate = Date.now() - statSync("./lib/chart_converter.ts").mtimeMs < 1000 || readdirSync("./.dev/level").length < levelList.length
  ...
  if (shouldUpdate) {
    ...
    levelList.forEach((level) => {
      ...
      writeFileSync(path.join(process.cwd(), `./.dev/level/${level.level_id}`), gzipSync(JSON.stringify(converted.data, null, 2)))
    })
