import { type } from "os"
import { archetypes } from "."
import { skin } from "../skin"
import { canvas, judgeLineX, lineToQuad, scaleX, scaleY, speed } from "./shared"

export class LinePoint extends Archetype {

  import = this.defineImport({
    Line: { name: "Line", type: Number },
    SpawnTime: { name: "SpawnTime", type: Number },
    Time: { name: "Time", type: Number },
    NextPoint: { name: "NextPoint", type: Number },
    YPos: { name: "YPos", type: Number },
    IsLastPoint: { name: "IsLastPoint", type: Number },
    CanvasId: { name: "CanvasId", type: Number },
    ColorA: { name: "ColorA", type: Number },
  })

  spawnTime = this.entityMemory(Number)
  nextTime = this.entityMemory(Number)
  endTime = this.entityMemory(Number)

  pos = this.defineSharedMemory({
    x: Number,
    y: Number
  })

  preprocess() {
    this.spawnTime = bpmChanges.at(this.import.SpawnTime).time - 10 / speed
    this.endTime = bpmChanges.at(this.import.Time).time
    this.nextTime = archetypes.LinePoint.import.get(this.import.NextPoint).NextPoint

  }

  shouldSpawn() {
    return (this.spawnTime <= time.now)
  }

  updateSequential() {
    const n = archetypes.LinePoint.pos.get(this.import.NextPoint)
    if (n.x >= judgeLineX || (this.import.IsLastPoint && this.endTime >= time.now)) this.despawn = true

    const debugLayout = Rect.one.mul(0.025)

    this.pos.x = scaleX(this.import.Time)
    this.pos.y = scaleY(this.import.YPos)

    const nx = archetypes.LinePoint.pos.get(this.import.NextPoint).x
    const ny = archetypes.LinePoint.pos.get(this.import.NextPoint).y

    const lineLayout = lineToQuad(this.pos.x, this.pos.y, nx, ny)



    if (this.import.IsLastPoint == 0 && archetypes.LinePoint.pos.get(this.import.NextPoint).y != 0) {
      skin.sprites.lineNeutral.draw(lineLayout, 3, this.import.ColorA)
    }
    skin.sprites.judgeRingBlue.draw(debugLayout.translate(this.pos.x, this.pos.y), 2, this.import.ColorA)

    // if (time.now >= this.endTime + 3) this.despawn = true
  }
}
